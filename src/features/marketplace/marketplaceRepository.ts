...
import type { CommissionDraft, PlayerShop } from './types';

const shopSchema = z.object({
  id: z.string(), owner_id: z.string(), owner_name: z.string(), owner_avatar: z.string().nullish(),
  discord_user_id: z.string().nullish(), kind: z.enum(['crafting', 'ritual']), title: z.string(),
...
function fromRow(value: unknown): PlayerShop {
  ...
  return {
    ...
    discordUserId: row.discord_user_id ?? undefined, kind: row.kind, title: row.title, description: row.description,
...
export async function saveShop(shop: Omit<PlayerShop, 'id' | 'ownerId' | 'ownerName' | 'ownerAvatar' | 'updatedAt'> & { id?: string }): Promise<string> {
