...
import { listMyShopCharacters, listShops, saveShop, submitCommission } from './marketplaceRepository';
import { assuranceDc, emptyBonus, proficiencyValues, totalBonus, type BonusBreakdown, type CommissionDraft, type PlayerShop, type ProficiencyRank, type ShopCharacterOption, type ShopKind } from './types';
import './marketplace.css';

const blankShop = (kind: ShopKind = 'crafting') => ({
  characterId: '', kind, title: '', description: '', imageUrl: '', discordUserId: '', tags: [] as string[], specialty: '', tier: 1,
...
export default function MarketplacePage() {
  ...
  return <main className="marketplace-shell">
    <header className="marketplace-hero">
      ...
      {isAuthenticated && <button className="marketplace-primary" onClick={() => setEditing(true)}><Plus /> {shops.some(shop => shop.ownerId === user?.id) ? 'Manage my shops' : 'Open a shop'}</button>}
    </header>
