...
const blankShop = (kind: ShopKind = 'crafting') => ({
  characterId: '', kind, title: '', description: '', imageUrl: '', discordUserId: '', discordPingsEnabled: false, tags: [] as string[], specialty: '', tier: 1,
...
function ShopDetail({ shop, authenticated, onClose }: { shop: PlayerShop; authenticated: boolean; onClose: () => void }) {
  ...
  return <div className="marketplace-modal" role="dialog" aria-modal="true"><article className="shop-detail">
    ...
    <p className="marketplace-kicker">{shop.kind} shop · Tier {shop.tier}</p><h1>{shop.title}</h1><p className="shop-owner">Operated by <b>{shop.characterName}</b> · Player {shop.ownerName}</p><p className="shop-description">{shop.description}</p>
...
function ShopEditor({ ownedShops, onClose, onSaved }: { ownedShops: PlayerShop[]; onClose: () => void; onSaved: () => void }) {
