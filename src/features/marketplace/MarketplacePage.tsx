...
import { assuranceDc, emptyBonus, proficiencyValues, totalBonus, type BonusBreakdown, type CommissionDraft, type CommissionStatus, type PlayerShop, type ProficiencyRank, type ShopCharacterOption, type ShopCommission, type ShopKind } from './types';
import './marketplace.css';
import './marketplaceQueue.css';

const blankShop = (kind: ShopKind = 'crafting') => ({
  characterId: '', kind, title: '', description: '', imageUrl: '', pageTheme: 'forge' as const, pageAccentColor: '#d1cabf', pageBackgroundImageUrl: '', pageTagline: '', discordUserId: '', discordPingsEnabled: false, tags: [] as string[], specialty: '', tier: 1,
...
export function CommissionForm({ shop, onClose, onSubmitted }: { shop: PlayerShop; onClose: () => void; onSubmitted?: () => void }) {
  ...
  if (status === 'done') return <div className="commission-form success"><Check /><h2>Request added to the queue</h2><p>The shop owner can now review it on the website. A Discord ping was also sent if they opted in.</p><button onClick={onClose}>Close</button></div>;
...
export function ShopEditor({ ownedShops, onClose, onSaved }: { ownedShops: PlayerShop[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="marketplace-modal"><form className="shop-editor" onSubmit={submit}><button type="button" className="modal-close" onClick={onClose}><X /></button><p className="marketplace-kicker">Shop profile</p><h1>{'id' in shop ? 'Manage this shop' : 'Open a shop'}</h1>
    <div className="form-grid"><label>Shop character<select required value={shop.characterId} onChange={e => chooseCharacter(e.target.value)}><option value="">Choose one of your characters</option>{characters.map(character => <option key={character.id} value={character.id}>{character.name} · level {character.level}</option>)}</select></label><label>Shop type<select value={shop.kind} onChange={e => setShop(current => ({ ...current, kind: e.target.value as ShopKind }))}><option value="crafting">Crafting</option><option value="ritual">Ritual</option></select></label><label>Title<input required minLength={3} maxLength={80} value={shop.title} onChange={e => set('title', e.target.value)} /></label><label className="span-2">Description<textarea required rows={4} maxLength={1200} value={shop.description} onChange={e => set('description', e.target.value)} /></label><label>Optional card / banner image URL<input type="url" value={shop.imageUrl ?? ''} onChange={e => set('imageUrl', e.target.value)} /></label><label>Tags, comma separated<input value={tagText} onChange={e => setTagText(e.target.value)} placeholder="weapons, alchemy, rush orders" /></label><label>Maximum tier<input type="number" min="1" max="20" value={shop.tier} onChange={e => set('tier', Number(e.target.value))} /></label><label>Overall discount %<input type="number" min="0" max="100" value={shop.overallDiscountPercent} onChange={e => set('overallDiscountPercent', Number(e.target.value))} /></label></div>
    <fieldset className="shop-page-settings"><legend>Shop page</legend><div className="form-grid"><label>Page theme<select value={shop.pageTheme} onChange={e => set('pageTheme', e.target.value)}><option value="forge">Forge</option><option value="arcane">Arcane</option><option value="parchment">Parchment</option></select></label><label>Accent color<input type="color" value={shop.pageAccentColor} onChange={e => set('pageAccentColor', e.target.value)} /></label><label className="span-2">Tagline<input maxLength={180} value={shop.pageTagline} onChange={e => set('pageTagline', e.target.value)} placeholder="Fine steel, impossible deadlines." /></label><label className="span-2">Background image URL<input type="url" value={shop.pageBackgroundImageUrl ?? ''} onChange={e => set('pageBackgroundImageUrl', e.target.value)} placeholder="https://..." /></label></div></fieldset>
    <fieldset className="discord-settings"><legend>Discord notifications</legend><label className="checkbox"><input type="checkbox" checked={shop.discordPingsEnabled} onChange={e => set('discordPingsEnabled', e.target.checked)} /> Ping me on Discord for new commissions</label>{shop.discordPingsEnabled && <label>Discord user ID<input required inputMode="numeric" pattern="[0-9]{17,20}" value={shop.discordUserId ?? ''} onChange={e => set('discordUserId', e.target.value)} placeholder="17–20 digit Discord user ID" /></label>}<p>Website queue notifications remain available whether Discord pings are enabled or not.</p></fieldset>
