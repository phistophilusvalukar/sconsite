...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp SET row_security = off AS $$
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'discord_user_id', s.discord_user_id, 'kind', s.kind, 'title', s.title, 'description', s.description,
    'image_url', s.image_url, 'tags', s.tags, 'specialty', s.specialty, 'tier', s.tier,
    'overall_discount_percent', s.overall_discount_percent, 'feats', s.feats, 'crafting_bonus', s.crafting_bonus,
    'crafting_assurance', s.crafting_assurance, 'crafting_degree_boost', s.crafting_degree_boost,
    'ritual_skills', s.ritual_skills, 'rituals', s.rituals, 'contributors', s.contributors,
    'accepts_commissions', s.accepts_commissions, 'updated_at', s.updated_at
  ) ORDER BY s.updated_at DESC), '[]'::jsonb) FROM public.player_shops s JOIN public.users u ON u.id = s.owner_id;
$$;
...
DECLARE actor_id uuid; result_id uuid; requested_id uuid; shop_kind text; shop_title text; shop_description text;
BEGIN
  ...
  IF requested_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.player_shops WHERE id=requested_id AND owner_id=actor_id) THEN RAISE EXCEPTION 'Shop not owned by caller'; END IF;
  INSERT INTO public.player_shops (id,owner_id,kind,title,description,image_url,discord_user_id,tags,specialty,tier,overall_discount_percent,feats,crafting_bonus,crafting_assurance,crafting_degree_boost,ritual_skills,rituals,contributors,accepts_commissions,updated_at)
  VALUES (COALESCE(requested_id,gen_random_uuid()),actor_id,shop_kind,shop_title,shop_description,NULLIF(p_shop->>'imageUrl',''),NULLIF(p_shop->>'discordUserId',''),ARRAY(SELECT jsonb_array_elements_text(COALESCE(p_shop->'tags','[]'))),NULLIF(p_shop->>'specialty',''),COALESCE((p_shop->>'tier')::int,1),COALESCE((p_shop->>'overallDiscountPercent')::int,0),COALESCE(p_shop->'feats','[]'),COALESCE(p_shop->'craftingBonus','{}'),COALESCE((p_shop->>'craftingAssurance')::boolean,false),COALESCE(p_shop->>'craftingDegreeBoost',''),COALESCE(p_shop->'ritualSkills','[]'),COALESCE(p_shop->'rituals','[]'),COALESCE(p_shop->'contributors','[]'),COALESCE((p_shop->>'acceptsCommissions')::boolean,true),now())
  ON CONFLICT (owner_id) DO UPDATE SET kind=EXCLUDED.kind,title=EXCLUDED.title,description=EXCLUDED.description,image_url=EXCLUDED.image_url,discord_user_id=EXCLUDED.discord_user_id,tags=EXCLUDED.tags,specialty=EXCLUDED.specialty,tier=EXCLUDED.tier,overall_discount_percent=EXCLUDED.overall_discount_percent,feats=EXCLUDED.feats,crafting_bonus=EXCLUDED.crafting_bonus,crafting_assurance=EXCLUDED.crafting_assurance,crafting_degree_boost=EXCLUDED.crafting_degree_boost,ritual_skills=EXCLUDED.ritual_skills,rituals=EXCLUDED.rituals,contributors=EXCLUDED.contributors,accepts_commissions=EXCLUDED.accepts_commissions,updated_at=now()
