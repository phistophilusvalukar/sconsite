/*
  # Keep database-admin guild rosters current

  Removed or transferred characters can leave historical membership rows behind.
  The current core roster requires both an active membership and a matching
  characters.guild_id, so derive admin candidates and counts from that source.
*/
...
BEGIN
  ...
  WHERE membership.guild_id = p_guild_id
    ...
    AND character.guild_id = p_guild_id;

  UPDATE public.guilds
  SET member_count = active_core_count,
      status = CASE
        WHEN status = 'Disbanded' THEN status
...
-- Repair counts that were cached before roster consistency was enforced.
  ...
  SELECT guilds.id,
    count(membership.id) FILTER (
      WHERE membership.membership_status = 'Active'
        AND membership.role_category IN ('Leader', 'Subleader', 'Officer', 'Member')
        AND character.character_status = 'active'
        AND character.guild_id = guilds.id
...
BEGIN
  ...
  SELECT character.name INTO new_leader_name
  FROM public.guild_memberships membership
  JOIN public.characters character ON character.id = membership.character_id
