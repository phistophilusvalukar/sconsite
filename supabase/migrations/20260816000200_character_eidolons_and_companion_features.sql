/* Add Eidolons and expose a small, display-only feature summary on public profiles. */
...
ALTER TABLE public.character_foundry_files
  ...
  ADD CONSTRAINT character_foundry_files_subject_type_check
    CHECK (subject_type IN ('character', 'familiar', 'animal_companion', 'eidolon'));

ALTER FUNCTION public.get_public_character_profile(uuid)
  RENAME TO get_public_character_profile_without_eidolons;
...
AS $$
  ...
  CROSS JOIN LATERAL (
    SELECT public.get_public_character_profile_without_eidolons(character.id) AS payload
  ) public_profile
  CROSS JOIN LATERAL (
    SELECT jsonb_agg(
      jsonb_build_object(
